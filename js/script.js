  ...
  if (page == "firstPage") {
    const addUniLogo = () => {
      doc.addImage(
        logoUni,
        "PNG",
        A4_WIDTH_SIZE / 2 - (uniW * 1.5) / 2 - ApVestW / 1.5,
        pxToPt(20),
        uniW * 1.5,
        uniH * 1.5,
        "",
    ...
    const addApVestLogo = () => {
      doc.addImage(
        logoApVest,
        "PNG",
        A4_WIDTH_SIZE / 2 + (uniW * 1.5) / 2 - ApVestW / 1.5 + cmToPt(1.8),
        pxToPt(80),
        ApVestW / 1.5,
        ApVestH / 1.5,
        "",
...
function executar() {
  ...
  reader.onload = () => {
    ...
    reader2.onload = () => {
      ...
      const alunosAprovados = encontrarAlunosAprovados(
        alunosData,
        aprovadosData
      );
      ...
      doc
        ...
        .setFontSize(15)
        .text(
          "Essa ferramenta está em desenvolvimento, seu nome não estar aqui NÃO significa que você não foi aprovado. Confira a lista de aprovados oficial.",
          A4_WIDTH_SIZE / 2,
          A4_HEIGHT_SIZE - cmToPt(4),
          {
            maxWidth: A4_WIDTH_SIZE - 2 * cmToPt(2),
            align: "center",
          }
